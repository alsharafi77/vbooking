...
String frequentFlyerParam     = request.getParameter("frequentflyer");
...
if (frequentFlyerParam == null) frequentFlyerParam = "";
...
<% if (flightNumberParam != null) { %>
<FORM id="bookingForm" name="bookingForm" ACTION="book-flight-controlFF.jsp" class='usesCurrencyCode'>
<%} %>
<table class="container">
	...
	<%if(flightOnly){ %>
		...
		<table class="block_searched" style="float: left; padding: 20px; margin-right: 5px; height: 600px;">
			...
			<% if (flightNumberParam == null) { %>
				<br>
				<b style="margin-left: 20px;">No flights found from <%=flyingFromParam%> to <%=flyingToParam%> on <%=from%></b>
				<br>
				<br>
			...
			<% } else { %>
				...
				if (!specialOffer) {
				%>
					<b><h1 style="margin-left: 20px;">Search results:</h1></b>
				...
				<% if((flights != null) && (flights.size() > 1)) { %>			
						...
						%>
						<option flightNumber="<%=fNumber%>" weekNumber="<%=fwNumber%>" departs="<%=fDeparts%>" arrives="<%=fArrives%>" fprice="<%=fPrice%>"
							<% if (i == 0) {%>selected<%} %>><%=fNumber%>-<%=fwNumber%></option>
					<% } %>
					</select>
			...
			<% } else if (hotels == null || hotels.size() == 0) { %>
	 			 <br><br>
	 			 <h1 style="margin-left: 20px;">No hotels available in <%=city%> </h1> 
	 			 <h3 style="margin-left: 20px;">from <%=from%> to <%=to%> </h3> 
	 			 <INPUT TYPE="hidden" NAME="selectedHotel" VALUE="none" checked>
			<%
			} else {
				hotelsFound = true;
			%> 
			<br><br>
			<h1 style="margin-left: 20px; white-space: nowrap;">Available hotels in <%=city%> </h1>
			<h3 style="margin-left: 20px;">from <%=from%> to <%=to%></h3>
			...
			<tr>
			<td>
				<INPUT TYPE="radio" NAME="selectedHotel" VALUE="<%=hotel.getName()%>" style="margin-left: 20px;" RATE='<%=hotel.getRate() %>'
							onclick="document.getElementById('totalCost').setAttribute('hotelPrice', '<%=hotel.getRate() %>');"\>
			</td>
			<td><%=hotel.getName()%></td>
			...
			<br> <font size=-2 style="margin-left: 20px;">* Hotel billed separately</font> 
			<%  
				}
			} else { 
			%> 
			<h1 style="margin-left: 20px;"> Find a hotel: </h1><br>
				...
				<td style="border: 0px">
					<input value="<%=from%>" maxlength="10" size="10" id="hotelFrom" name="hotelFrom" type="text"> 
					<a href="javascript:show_calendar('bookingForm.hotelFrom', document.getElementById('hotelFrom').value);" 
							onmouseover="window.status='Date Picker';return true;" onmouseout="window.status='';return true;"> 
					<img src="graphics/new/gh_demo_calendar.png" class="calendar" border="0" STYLE="position: relative; top: 10px; left: -1px;">
					</a>
				</td>
				</tr>
				<tr>
				...
				<td style="border: 0px">
					<input value="<%=to%>"maxlength="10" size="10" id="hotelTo" name="hotelTo" type="text"> 
					<a href="javascript:show_calendar('bookingForm.hotelTo', document.getElementById('hotelTo').value);"
							onmouseover="window.status='Date Picker';return true;" onmouseout="window.status='';return true;"> 
					<img  src="graphics/new/gh_demo_calendar.png" class="calendar" border="0" STYLE="position: relative; top: 10px; left: -1px;">
					</a>
				</td> 
		...
		<!-- Customer table -->
			...
			<TD valign="top">
				...
				<%
					...
					String pPostalCode = ""; %>
					<option pFirstName="<%=pFirstName%>" pMiddleName="<%=pMiddleName%>" pLastName="<%=pLastName%>" pGender="<%=pGender%>" pFF="<%=pFF%>"
						pCardType="<%=pCardType%>" pCardHolderName="<%=pCardHolderName%>" pCardNumber="<%=pCardNumber%>" pCardSecurityCode="<%=pCardSecurityCode%>"
				 		pAddressLine1="<%=pAddressLine1%>" pAddressLine2="<%=pAddressLine2%>" pPostalCode="<%=pPostalCode%>" selected>Autofill Customer ></option>
					<%
					try {
		        ...
		        %>
		        	<option pFirstName="<%=pFirstName%>" pMiddleName="<%=pMiddleName%>" pLastName="<%=pLastName%>" pGender="<%=pGender%>" pFF="<%=pFF%>"
		        		pCardType="<%=pCardType%>" pCardHolderName="<%=pCardHolderName%>" pCardNumber="<%=pCardNumber%>" pCardSecurityCode="<%=pCardSecurityCode%>"
		        		pAddressLine1="<%=pAddressLine1%>" pAddressLine2="<%=pAddressLine2%>" pPostalCode="<%=pPostalCode%>"><%=pFirstName%>
		        	</option>
		        <%
	   	 		}
				} catch (Exception e) {
			...
			<SELECT class="booked_set" style="width: 150px; background-color: #003a8c; margin-top: 5px; margin-left: 20px;" NAME="passengergender">
					...
					<OPTION VALUE="M" <% if ("M".equals(passengerGenderParam)) { %> selected <% } %>>Male</OPTION>
					<OPTION VALUE="F" <% if ("F".equals(passengerGenderParam)) { %> selected <% } %>>Female</OPTION>
			</SELECT>
			<INPUT class="PassengerText" style="margin-top: 5px; margin-bottom: 5px;" TYPE="TEXT" NAME="frequentflyer"
							VALUE="<%=frequentFlyerParam%>" placeholder=" Frequent Flyer" size="20"\>
			<BR></TD>
			</TR>
	</table>
	</TD>
	...
	<table class="block_searched" style="float: left; padding: 20px; margin-left: 5px;">
		...
		<TD valign="top">
			...
			<SELECT class="booked_set" style="width: 150px; background-color: #003a8c; margin-left: 20px; margin-bottom: 5px;" NAME="cardtype">
					...
					<OPTION VALUE="multinational" <% if ("multinational".equals(cardTypeParam)) { %> selected <% } %>>Multinational</OPTION>
					<OPTION VALUE="global" <% if ("global".equals(cardTypeParam)) { %> selected <% } %>>Global</OPTION>
					<OPTION VALUE="worldwide" <% if ("worldwide".equals(cardTypeParam)) { %> selected <% } %>>Worldwide</OPTION>
			</SELECT>
