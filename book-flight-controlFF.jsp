...
if (paramError!=null) {
    ...
    RequestDispatcher dispatcher = request.getRequestDispatcher("errorFF.jsp");
...
Element frequentFlyerElement = new Element("frequentFlyer");
frequentFlyerElement.setText(frequentFlyerParam);
bookFlightRequestElement.addContent(frequentFlyerElement);
...
String requestXMLforServer = xslTransformer.transform("BookFlightRequestFF.xsl", requestXMLdata);
...
synchronized (theJMSmanager) {
	try {
		...
		String replyQueueName = "vbooking.request.passenger.booking.ff.reply";
		if (theJMSmanager.getRequestQueueName()==null ||
		    !theJMSmanager.getRequestQueueName().equals(queueName) ||
		    !theJMSmanager.getReplyQueueName().equals(replyQueueName)) {
			//Make sure the correct queue names are set in the JMSManager singleton
			theJMSmanager.initQueueSession(queueName, replyQueueName);
		}
		
		xmlResponse = theJMSmanager.doJMSrequest(requestXMLforServer); //Fire JMS Request!
	} catch (NullPointerException e) {
		String error_response = "An error occurred configuring JMS. This typically happens when the Websphere MQ service is not running, please check your Websphere MQ installation is configured correctly and running."; 
	    %><link rel="stylesheet" type="text/css" href="style.css" ><h1>MQ RESPONSE FAILURE</h1>
		  <PRE><%=org.apache.commons.lang.StringEscapeUtils.escapeHtml(error_response)%></PRE><%
	} catch (JMSManagerException e) { // likely a time out
		String error_message = e.getMessage();
		String error_response;
		if (error_message.equals("JMS initialisation error")) {
			error_response = error_message + ". Typically occurs when Websphere MQ is not running or configured incorrectly. Please check Websphere MQ is running and the queues configured correctly.";;
		} else if (error_message.equals("JMS timeout error")) {
			error_response = error_message + ". Typically occurs when queue messages are not being processed. Please check the queue messages are being processed and WebSphere Application Server is configured correctly and running."; 
		} else {
			error_response = error_message + ". Please check queue messages are being processed and the WebSphere Application Server is configured correctly and running."; 
		}
	    %><link rel="stylesheet" type="text/css" href="style.css" ><h1>JMS MANAGER ERROR</h1>
		  <PRE><%=org.apache.commons.lang.StringEscapeUtils.escapeHtml(error_response)%></PRE><%		 
	} catch (Exception e) {
		StringWriter e_errors = new StringWriter();
		e.printStackTrace(new PrintWriter(e_errors));

		String error_response = "An unexpected error occurred contacting the message queue, please check your Websphere MQ configuration.\n" 
...
if (!xmlResponse.isEmpty()) {// there is a response
	...
	if (status.startsWith("SUCCESS")) {
	    ...
	    String loyaltyPoints = JDOMUtils.getDataField(responseDocument, "/bookFlightResponse/loyaltyPoints", request, response);
		
	    RequestDispatcher dispatcher = request.getRequestDispatcher("booking-madeFF.jsp?reservationnumber=" + reservationNumber + "&loyaltypoints=" + loyaltyPoints + "&destination=" + flyingToParam + "&from=" + from + "&to=" + to + "&city=" + city);
	    dispatcher.forward(request, response);
